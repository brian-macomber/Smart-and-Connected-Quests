...
#define ESP_IP_ADDR "192.168.7.245"

#define PORT 1131
#define PORT_ESP 1132
...
static void udp_client_task_node(void *pvParameters)
{
    char rx_buffer[128];
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            // instead of sending payload, we send the temp!
...
static void udp_client_task_secondaryESP(void *pvParameters)
{
    char rx_buffer[128];
    char host_ip[] = ESP_IP_ADDR;
    int addr_family = 0;
    int ip_protocol = 0;
    ...
    while (1)
    {
        ...
        struct sockaddr_in dest_addr;
        dest_addr.sin_addr.s_addr = inet_addr(ESP_IP_ADDR);
        dest_addr.sin_family = AF_INET;
        dest_addr.sin_port = htons(PORT_ESP);
        addr_family = AF_INET;
        ip_protocol = IPPROTO_IP;
        ...
        ESP_LOGI(UDP_TAG, "Socket created, sending to %s:%d", ESP_IP_ADDR, PORT_ESP);
        ...
        while (1)
        {
            ...
            if (start_button || start_web)
            {
                strcpy(new_payload, "Start");
            }
            else if (count >= 0 && count < 2)
            {
                strcpy(new_payload, "Toast");
            }
            else if (rst_button || rst_web)
            {
                strcpy(new_payload, "Reset");
            }
            else
            {
                strcpy(new_payload, "NULL");
            }

            int err = sendto(sock, new_payload, strlen(new_payload), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
            if (err < 0)
            {
                ESP_LOGE(UDP_TAG, "Error occurred during sending: errno %d", errno);
                break;
            ...
            if (strcmp(rx_buffer, "Stop!") == 0) // if there is no toast in toaster, stop counter
            {
                rst_web = 1;
            }
...
void app_main(void)
{
    ...
    xTaskCreate(udp_client_task_node, "udp_client1", 4096, NULL, 5, NULL);
    xTaskCreate(udp_client_task_secondaryESP, "udp_client2", 4096, NULL, 5, NULL);
    ...
    while (1)
    {
        ...
        tempCelc = (tempKelvin - 273.15);
        printf("%f\n", tempCelc);
        //put temp into payload
        sprintf(payload, "%.2f\n", tempCelc);
